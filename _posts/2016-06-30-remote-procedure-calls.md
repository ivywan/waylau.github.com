...
在远程过程调用步骤
让我们检查本地过程调用是如何实现的。这个编译器和架构之间的不同,所以我们将概括。每个处理器为我们提供了某种形式的调用指令,它把堆栈上的下一个指令的地址,将控制转给指定的地址电话。被调用过程完成后,它会返回指令,从堆栈的顶部弹出地址和传输控制。这只是基本的处理器机制,使得它容易实现过程调用。识别的实际细节参数,将它们放在堆栈,执行一个指令的调用编译器。在调用函数时,编译器负责确保保存任何寄存器被拖垮,分配堆栈空间的局部变量,然后在返回之前恢复寄存器和堆栈。

这一切都将调用一个远程机器上加载过程。这意味着编译器必须做些不同的提供远程过程调用的假象。这使得远程过程调用一个语言级构造与套接字、一个操作系统级构造。我们将不得不模拟远程过程调用工具,我们有,即本地过程调用和网络通信的套接字。

整个技巧,远程过程调用的工作是创建存根函数,使其出现的用户是本地电话。在客户端,一个存根函数看起来像用户打算调用的函数实际上包含代码通过网络发送和接收消息。发生的操作序列,如图1所示(p。693 w·理查德·史蒂文的UNIX网络编程),是:

图1所示。步骤执行远程过程调用
图1所示。步骤执行远程过程调用
客户端调用本地程序,称为客户端存根。客户端进程,这似乎是实际的过程,因为它是一个普通的本地过程。只是做了一些不同的服务器上的实际过程。客户端存根包远程过程的参数(这可能涉及到将它们转换为一个标准的格式)和构建一个或多个网络消息。的参数打包成网络消息叫做封送和需要序列化成平面的字节数组格式的所有数据元素。

网络消息被发送到客户端存根远程系统(通过系统调用本地内核使用套接字接口)。

由内核网络消息传输到远程系统通过一些协议(面向连接或无连接)。

服务器存根,有时被称为骨架,在服务器上接收消息。它对这个参数的信息,如果有必要,将它们从一个标准的网络格式转换成特定形式。

服务器存根调用服务器功能(客户端,远程过程),通过它的参数收到客户端。

当服务器函数完成时,它返回到服务器存根返回值。

服务器存根将返回值,如果有必要,和排列成一个或多个网络消息发送给客户端存根。

消息会通过网络发送回客户端存根。

客户端存根读取信息从本地内核。

客户端存根返回结果给客户端函数,将他们从网络表示转换为本地如果必要的话。

客户端代码然后继续执行。

RPC的主要好处是双重的。首先,程序员可以使用调用远程过程调用语义功能和反应。其次,编写分布式应用程序简化,因为RPC隐藏了所有的网络代码存根函数。应用程序不必担心一些细节,比如套接字、端口号和数据转换和解析。在OSI参考模型,RPC跨会话和表示层(层5和6)。

实现远程过程调用
当我们想到,会产生几个问题实现远程过程调用。

如何传递参数?
通过值传递很简单:复制到网络消息的值。按引用传递是困难的。没有意义的地址传递给远程计算机以来,内存位置可能指向远程系统上的一些完全不同的。如果你想支持按引用传递,你必须发送一个副本的争论,将它们在内存中远程系统,向他们传递一个指针到服务器的功能,然后将对象发送回客户端,复制它的引用。如果远程过程调用必须支持引用复杂的结构,如树链表,他们将不得不(如结构复制到一个pointerless表示。,一个扁平的树),传输,在远程端重建数据结构。

我们如何表示数据?
商店最重要的字节的大端字节表示一个多字节整数在低内存。小端字节表示商店最重要的高端内存的字节的整数。机器如Sun sparc和旧的摩托罗拉(Motorola 680 x0大端字节存储使用。大多数英特尔系统使用小端字节存储。许多其他体系结构使用bi-endian格式,处理器可以在启动时配置在小端字节或大端字节模式。这些处理器的例子有手臂,MIPS、PowerPC、SPARC v9,英特尔ia - 64(安腾)。
在本地系统上没有数据不相容问题,数据格式总是相同的。与RPC远程机器上可能有不同的字节顺序,不同大小的整数,浮点表示不同。问题是处理IP协议套件,迫使每个人都使用大端字节字节次序16和32位字段标题(因此使用htons和htonl功能)。对于RPC,我们需要想出一个“标准”对所有数据类型编码,可以作为参数传递,如果我们要与异构系统通信。ONC RPC,例如,使用格式XDR(外部数据表示)呼吁这一过程。这些数据表示格式可以使用隐式或显式类型。隐式类型,只有价值传播,而不是变量的名称或类型。ONC RPC的XDR和DCE RPC的NDR的例子使用隐式类型的数据表示。显式类型,每个字段的类型以及传播价值。ISO标准的ASN。1(抽象语法符号),JSON(JavaScript对象表示法),谷歌协议缓冲区,和各种基于xml数据表示格式使用显式类型。

我们机器和端口绑定到什么呢?
我们需要找到一个远程主机和适当的过程,主机上(端口或运输地址)。一个解决方案是维护一个集中的数据库,可以找到一个主机提供的一种服务。这个方法提出了Birell和尼尔森在他们一份1984年的论文介绍RPC。服务器发送一条消息到中央机关陈述其愿意接受一定的远程过程调用。然后联系客户这中央权威当他们需要找到一个服务。另一个解决方案,那么优雅但更容易管理,是要求客户端知道哪些主机需要接触。一个名称服务器主机维护一个数据库提供的本地服务。

传输协议应该使用什么?
有些实现只允许一个使用(例如TCP)。大多数RPC实现支持几个和允许用户选择。

当事情出错时,会发生什么?
现在有更多的错误的机会。一个服务器可以生成一个错误,在网络上可能会有问题,服务器可能会崩溃,或者客户端可以在服务器运行时代码消失。远程过程调用的透明度,打破了这里因为本地过程调用没有过程调用的失败的概念。正因为如此,项目使用远程过程调用必须准备测试远程过程调用的失败或捕获到一个异常。

远程调用的语义是什么?
调用一个普通的语义过程很简单:执行过程当我们叫它。远程过程,完全一旦方面非常难以实现。可以执行远程过程:

0 *如果服务器崩溃或流程在运行服务器代码之前去世。
如果一切工作正常。
一次或更多如果服务器返回服务器存根后坠毁之前发送响应。客户端不会返回响应,可以决定再试一次,因此多次执行的函数。如果没有再试一次,函数执行一次。
不止一次如果客户机超时和重新传输。有可能是原始请求可能会被推迟。都可能会执行(或没有)。
RPC系统通常会提供至少一次或它们之间的语义或选择最多一次。一个需要了解应用程序的性质和功能的远程过程来确定是否安全的可能不止一次调用一个函数。如果一个函数可以运行任何次数没有伤害,这是幂等(如。每天的时间,数学函数,读取静态数据)。否则,它是一个nonidempotent函数(如。添加或修改一个文件)。
